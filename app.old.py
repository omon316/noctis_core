"""
NoctisCore - Flask Backend
==========================

Custom Dashboard mit Flask für volle Design-Kontrolle
Mit integrierten Stats, Logs und Map APIs
"""
...
def stats_top_devices():
    ...
    time_filter = request.args.get('filter', None)
...
def api_stats_overview():
    """Overview Metrics"""
    from api.stats_api import get_overview_stats
    return jsonify(get_overview_stats())

@app.route('/api/stats/top-devices')
def api_stats_top_devices():
    """Top Devices"""
    from api.stats_api import get_top_devices_data
    time_filter = request.args.get('time_filter')
    return jsonify(get_top_devices_data(10, time_filter))

@app.route('/api/stats/hourly')
def api_stats_hourly():
    """Hourly Activity"""
    from api.stats_api import get_hourly_stats
    time_filter = request.args.get('time_filter')
    return jsonify(get_hourly_stats(time_filter))
...
def api_stats_daily():
    """Daily Activity"""
    from api.stats_api import get_daily_stats
    time_filter = request.args.get('time_filter')
    return jsonify(get_daily_stats(30, time_filter))

@app.route('/api/stats/weekday')
def api_stats_weekday():
    """Weekday Pattern"""
    from api.stats_api import get_weekday_stats
    time_filter = request.args.get('time_filter')
    return jsonify(get_weekday_stats(time_filter))
...
def api_stats_heatmap():
    """Activity Heatmap"""
    from api.stats_api import get_activity_heatmap
    time_filter = request.args.get('time_filter')
    return jsonify(get_activity_heatmap(time_filter))
...
def api_stats_extended():
    ...
    # Falls du Option B gewählt hast:
